...
# Contants for number limits of data types in C
const MAX_7B = 1 << 7
const MAX_8B = 1 << 8
const MAX_15B = 1 << 15
const MAX_16B = 1 << 16
...
#var magenta_filter = preload("res://shaders/e")
var mapdata = {}
@export_file("*.map") var map_path = "res://maps/de_dust2.map":
	set(path):
		map_path = path
		self.clear()
...
func unpack_map():
	...
	for x in range(mapdata.width+1):
		for y in range(mapdata.height+1):
			...
			self.set_cell(map_coords, 0, tile_coords, rotation_offset)
